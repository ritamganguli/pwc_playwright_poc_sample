...
Before(async function (this: ICustomWorld, scenario: ITestCaseHookParameter) {
  ...
  this.testName = scenario.pickle.name.replace(/\W/g, "-");
  // customize the [browser context](https://playwright.dev/docs/next/api/class-browser#browsernewcontextoptions)
  this.context = await browser.newContext({
    acceptDownloads: true,
    recordVideo: { dir: "videos/", size: { width: 640, height: 480 } },
  ...
  await new Report().startReport(null, scenario);
});
...
After(async function (
  ...
  await new Report().endReport(null, scenario);
